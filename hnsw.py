...
class HNSW:
    ...
    def _search_layer(self, query: np.ndarray, entry_points: List[int],
        ...
        Uses two heaps:
        - candidates: min-heap by distance (explore closest nodes first)
        - w: max-heap by distance via negation (track ef best; w[0] = furthest result)
        """
        visited = set()
        candidates = []  # min-heap: (dist, node) — explore closest first
        w = []           # max-heap: (-dist, node) — w[0] is the furthest in result set
        ...
        while candidates:
            current_dist, current = heapq.heappop(candidates)  # closest candidate

            furthest_in_w = -w[0][0]  # distance of furthest element in result set
